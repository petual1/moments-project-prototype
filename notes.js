const addBox = document.querySelector(".add-box"),
    ...
    deadlineTag = popupBox.querySelector("input[type='date']"),
...
function merge(left, right) {
    ...
    while (leftIndex < left.length && rightIndex < right.length) {
        ...
        if (leftDeadline > rightDeadline) {
            ...
            // Se os prazos forem iguais, priorize a nota com a menor prioridade
...
function showNotes() {
    ...
    // Sort notes using merge sort
    notes = mergeSort(notes);

    document.querySelectorAll(".note").forEach(li => li.remove());
    notes.forEach((note, id) => {
        let filterDesc = note.description.replaceAll("\n", '<br/>');
        // Convert deadline date to the desired format
        let deadlineDate = note.deadline ? new Date(note.deadline + "T00:00:00Z") : null;
        let formattedMonth = months[deadlineDate.getUTCMonth()];
        let formattedDate = deadlineDate.getUTCDate();
        let formattedYear = deadlineDate.getUTCFullYear();
...
addBtn.addEventListener("click", e => {
    ...
    if (title || description) {
        ...
        let noteInfo = {
            title,
            description,
            priority: notes.length + 1, // Define a prioridade com base no número atual de notas
            deadline: deadline || null, // Define o prazo como nulo se não for fornecido
            date: `${month} ${day}, ${year}`
        }
        if (!isUpdate) {
            notes.push(noteInfo);
